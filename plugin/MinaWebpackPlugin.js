...
function _inflateEntries(entries = [], dirname, entry) {
  ...
  if(pages) {
    miniRootPath = path.dirname(entry)
    // console.log("miniRootPath", miniRootPath);
  }
  pages && pages.forEach(item => inflateEntries(entries, dirname, item))
  usingComponents && Object.values(usingComponents).forEach(item => {
    // 如果是绝对路径 则拼接跟路径
    // const componentPath = path.dirname(entry)
    if(item.indexOf("/") === 0){
      item = path.join(miniRootPath, "." + item)
    }
    inflateEntries(entries, dirname, item)
  })

  subpackages && subpackages.forEach(subpackage =>
